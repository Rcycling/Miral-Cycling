...
export default function CartPage() {
  ...
  const handleRemove = (item: { id: string; size: string }) => {
    dispatch({ type: 'REMOVE_ITEM', payload: { id: item.id, size: item.size } });
  };

  const handleQuantity = (item: { id: string; size: string; quantity: number }, qty: number) => {
    dispatch({
      type: 'UPDATE_QUANTITY',
      payload: { id: item.id, size: item.size, quantity: qty },
    });
  };
